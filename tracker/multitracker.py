...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, label, buffer_size=30, feat=None):
        ...
        if feat is None:
            self.temp_feat = [self.label - 1]
        else:
            self.temp_feat = feat


        self.update_features(self.temp_feat)
...
class JDETracker(object):
    def __init__(self, max_age=10, buffer_size=30, det_thresh=0.5, thresh1=0.7, thresh2=0.5, thresh3=0.7):
    ...
    def update(self, dets, features):
        ...
        if len(dets) > 0:
            detections = [STrack(STrack.tlbr_to_tlwh(x[:4]), score=x[4], label=x[5], buffer_size=self.buffer_size, feat=feat) for
                          (x, feat) in zip(dets, features)]
        else:
            detections = []
        ...
        matches, u_track, u_detection = matching.linear_assignment(dists, thresh=self.thresh1)
        ...
        matches, u_track, u_detection = matching.linear_assignment(dists, thresh=self.thresh2)
        ...
        matches, u_unconfirmed, u_detection = matching.linear_assignment(dists, thresh=self.thresh3)
        for itracked, idet in matches:
