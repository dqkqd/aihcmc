...
def generate_detections(
        cls_outputs, preds, box_outputs, anchor_boxes, indices, classes, img_size, img0_size,
        conf_thres:float, iou_thres:float, max_det_per_image: int = MAX_DETECTIONS_PER_IMAGE):
    ...
    boxes = clip_boxes_xyxy(boxes, img_size)  # clip before NMS better?

    scores = cls_outputs.sigmoid().squeeze(1).float()
    
    xi = scores > conf_thres
    boxes = boxes[xi]
    scores = scores[xi]
    classes = classes[xi]
    preds = preds[xi]
    ...
    top_detection_idx = batched_nms(boxes, scores, classes, iou_threshold=iou_thres)
    # # keep only topk scoring predictions
    top_detection_idx = top_detection_idx[:max_det_per_image]
    boxes = boxes[top_detection_idx]
    scores = scores[top_detection_idx, None]
    classes = classes[top_detection_idx, None]
    preds = preds[top_detection_idx].sigmoid().float()
    

    boxes[:, 0:4:2] = boxes[:, 0:4:2] * img0_size[1] / img_size
    boxes[:, 1:4:2] = boxes[:, 1:4:2] * img0_size[0] / img_size

    classes += 1  # back to class idx with background class = 0

    # stack em and pad out to MAX_DETECTIONS_PER_IMAGE if necessary
    detections = torch.cat([boxes, scores, classes.float(), preds], dim=1)
