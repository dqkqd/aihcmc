...
def _post_process(config, cls_outputs, box_outputs):
    ...
    cls_outputs_all_after_topk1 = torch.gather(
        cls_outputs_all, 1, indices_all.unsqueeze(2).expand(-1, -1, config.num_classes))
    cls_outputs_all_after_topk = torch.gather(
        cls_outputs_all_after_topk1, 2, classes_all.unsqueeze(2))

    return cls_outputs_all_after_topk, box_outputs_all_after_topk, indices_all, classes_all, cls_outputs_all_after_topk1


@torch.jit.script
def _batch_detection(batch_size: int, class_out, preds, box_out, anchor_boxes, indices, classes, img_size, img0_size, conf_thres:float, iou_thres:float):
    batch_detections = []
    # FIXME we may be able to do this as a batch with some tensor reshaping/indexing, PR welcome
    for i in range(batch_size):
        detections = generate_detections(
            class_out[i], preds[i], box_out[i], anchor_boxes, indices[i], classes[i], img_size[i], img0_size, conf_thres=conf_thres, iou_thres=iou_thres)
...
class DetBenchPredict(nn.Module):
    ...
    def forward(self, x, img_size, img0_size, conf_thres:float, iou_thres:float):
        class_out, box_out = self.model(x)
        class_out, box_out, indices, classes, preds = _post_process(self.config, class_out, box_out)
        return _batch_detection(
            x.shape[0], class_out, preds, box_out, self.anchors.boxes, indices, classes, img_size, img0_size, conf_thres, iou_thres)
